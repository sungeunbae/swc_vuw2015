...
*   `code/gen-world-database.sql`: re-generate world database used in examples.
*   `code/sqlitemagic.py`: IPython Notebook plugin to handle SQLite databases.
*   `data/*.csv`: CSV versions of data in sample world database.
...
Next,
download the SQL file `world.sql` from [http://files.software-carpentry.org/world.sql](http://files.software-carpentry.org/world.sql)
and the database file `world.db` from [http://files.software-carpentry.org/world.db](http://files.software-carpentry.org/world.db)
...
the most likely cause is that the database `world.db` isn't in the directory where you're running the command.
In this case,
...
run SQLite on `world.db` *without* the `.schema` command:
...
seq  name             file
---  ---------------  ----------------------------------------------------------
0    main             /Users/alan_turing/swc_sql/world.db
~~~
